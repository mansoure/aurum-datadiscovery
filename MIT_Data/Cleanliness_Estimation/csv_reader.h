...
private:
 	...
 	int csv_read_row(std::istream &in, std::vector<std::string> &row);
 	int get_number_of_rows(std::istream &in);
	int get_table(const string &filepath, vector<string> &header, doubleVecStr &data);
public:
	CSV_READER() {}
	// Table T;
